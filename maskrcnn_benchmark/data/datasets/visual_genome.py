...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        self.num_obj = 150
        # self.closed_classes = torch.randperm(len(self.ind_to_classes))
     
        self.closed_classes = torch.load('/data/hetao/eccv/Scene-Graph-Benchmark.pytorch/closed_words.pk')[:self.num_obj]

        self.evaluation_ov = True
        # if  self.evaluation_ov:
        #     t = []
        #     for i in self.categories:
        #         if i not in self.closed_classes:
        #             t.append(i)
        #     self.closed_classes = t
        if self.is_pretrain:
            self.region_descriptions = load_region_descriptions('/data/hetao/vg2/region_descriptions.json')
    ...
    def __getitem__(self, index):
        ...
        if img.size[0] != self.img_info[index]['width'] or img.size[1] != self.img_info[index]['height']:
            print(
                '=' * 20, ' ERROR index ', str(index), ' ', str(img.size), ' ', str(self.img_info[index]['width']), ' ',
        ...
        if not self.is_pretrain:
            ...
            target = self.get_region_groundtruth(index, flip_img)
        if flip_img:
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        if  self.closed_classes.size(0) >=150:
        ...
        box_cl = []
        gt_ob = []
        full_gt_class = self.gt_classes[index]
        remain_idx = []
        for idx, (bx, id) in enumerate(zip(target.bbox, full_gt_class)):
            if id in self.closed_classes:
                box_cl.append(bx)
                gt_ob.append(id)
                remain_idx.append(idx)
        if len(box_cl) <= 1:
            box = box[0].view(1,-1)
            target2 = BoxList(box, (w, h), 'xyxy')  # xyxy
            return target2
        box = torch.stack(box_cl, 0)
        remain_idx = torch.LongTensor(remain_idx)
        target2 = BoxList(box, (w, h), 'xyxy')  # xyxy
        target2.add_field("labels", torch.from_numpy(np.array(gt_ob)))

        target2.add_field("attributes",  target.get_field('attributes')[:len(target2)])
        rel_map = target.get_field('relation')
        cnt = 0
        rel_map = rel_map[remain_idx,:]
        rel_map = rel_map[:,remain_idx]
        # for i in range(rel_map.shape[0]):
        #     if full_gt_class[i] not in self.closed_classes:
        #         rel_map=rel_map[torch.arange(rel_map.size(0))!=(i-cnt)]
        #         cnt += 1
        # cnt = 0
        # for j in range(rel_map.shape[1]):
        #         if full_gt_class[j]  not in self.closed_classes:
        #             rel_map=rel_map[:,torch.arange(rel_map.size(1))!=(j-cnt)]
        #             cnt += 1
        if rel_map.sum() <= 0:
            box = box[0].view(1,-1)
            target2 = BoxList(box, (w, h), 'xyxy')  # xyxy
            return target2
        target2.add_field("relation", rel_map, is_triplet=True)
        # print(target2.get_field("labels").shape,target2.get_field("relation").shape,target2.get_field("relation").sum(),"+++")
        ...
        #     relation = [(k[0], k[1], np.random.choice(v)) for k,v in all_rel_sets.items()]
        #     relation = np.array(relation, dtype=np.int32)
        index = torch.nonzero(rel_map)
        rel_gt = rel_map[index[:, 0], index[:, 1]]
        relation = torch.cat([index, rel_gt.view(-1,1)],-1)
        # target2.add_field("relation_tuple", torch.LongTensor(relation))  # for evaluation

        # rel = target.get_field('relation_tuple')
        # rel2 = []
        # for i in range(rel.shape[0]):
        #     if rel[i][0] not in removed_id and rel[i][1] not in removed_id:
        #         rel2.append([id2id[rel[i][0]], id2id[rel[i][1]], rel[i][2]])
        # target.add_field("relation_tuple", torch.LongTensor(relation))  # for evaluation
        if  self.split in ['val','test']:
            target2 = target2.clip_to_image(remove_empty=False)
            target2.add_field("relation_tuple", torch.LongTensor(relation))  # for evaluation
            return target2
        else:

            target2 = target2.clip_to_image(remove_empty=True)
            return target2

    def remove_duplicates(self, phrases, max_num_rois=50):
        mp = {}
        ...
        for k, v in mp.items():
            if len(v) > 1:
                random.shuffle(v)
            index.append(v[0])
        if len(index) > max_num_rois:
            random.shuffle(index)
            index = index[:max_num_rois]
        return index

    def sort_by_x(self, boxes):
        c_x = 0.5 * (boxes[:, 0] + boxes[:, 2])
        perm = np.argsort(c_x)
        return perm

    def get_region_groundtruth(self, index, evaluation=False, flip_img=False):

        img_info = self.get_img_info(index)
        w, h = img_info['width'], img_info['height']
        img_id = img_info['image_id']
        reg_des = self.region_descriptions[img_id]
        box = np.array(reg_des['boxes'])
        phrases = reg_des['description']
        if self.split == 'train':
            selected_index = self.remove_duplicates(phrases)
            box = box[selected_index]
            phrases = [phrases[i] for i in selected_index]
        decend_sorted_indexes = self.sort_by_x(box)
        box = box[decend_sorted_indexes]
        phrases = [phrases[i] for i in decend_sorted_indexes]

        box = torch.from_numpy(box)

        target = BoxList(box, (w, h), 'xyxy')  # xyxy

        tokens = clip.tokenize(phrases, context_length=12, truncate=True)
        target.add_field("tokens", tokens)
...
def load_region_descriptions(path_file):
    ...
    for meta_im in tqdm(reg_des):
        box, phrase = process_region_descriptions(meta_im['regions'])
        imgs[meta_im['id']] = {}
        imgs[meta_im['id']]['boxes'] = box
        imgs[meta_im['id']]['description'] = phrase
