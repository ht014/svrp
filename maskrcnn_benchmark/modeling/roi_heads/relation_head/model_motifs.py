...
class LSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.obj_ctx_rnn = torch.nn.LSTM(
                input_size=self.obj_dim*1+self.embed_dim + 128,
        ...
        self.decoder_rnn = DecoderRNN(self.cfg, self.obj_classes, embed_dim=self.embed_dim,
                inputs_dim=self.hidden_dim + self.obj_dim *1 + self.embed_dim + 128,
                hidden_dim=self.hidden_dim,
                rnn_drop=self.dropout_rate)
        self.edge_ctx_rnn = torch.nn.LSTM(
                input_size=self.embed_dim + self.hidden_dim + self.obj_dim *1,
                hidden_size=self.hidden_dim,
        ...
        if self.effect_analysis:
            self.register_buffer("untreated_dcd_feat", torch.zeros(self.hidden_dim + self.obj_dim*1 + self.embed_dim + 128))
            self.register_buffer("untreated_obj_feat", torch.zeros(self.obj_dim*1+self.embed_dim + 128))
            self.register_buffer("untreated_edg_feat", torch.zeros(self.embed_dim + self.obj_dim*1))
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        if (all_average or ctx_average) and self.effect_analysis and (not self.training):
            ...
            obj_rel_rep = cat(( x, obj_embed2, obj_ctx), -1)
