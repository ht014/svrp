...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.only_vis = True
        self.lin_obj = nn.Linear(self.in_channels * self.only_vis + self.embed_dim*(1-self.only_vis) + 128, self.hidden_dim)
    ...
    def forward(self, roi_features, proposals, logger=None):
        ...
        if self.mode == 'predcls':
            ...
            edge_pre_rep = cat((obj_feats, self.obj_embed2(obj_labels.long())), dim=-1)
        else:
            ...
            # edge_pre_rep = cat((roi_features, obj_feats), dim=-1)
            edge_pre_rep = cat((obj_feats, self.obj_embed2(obj_preds)), dim=-1)
