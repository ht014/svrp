...
class TransformerContext(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.only_vis = True
        self.lin_obj = nn.Linear(4096 + 128, self.hidden_dim)
        self.lin_edge = nn.Linear(512+128, self.hidden_dim)
        # self.lin_edge = nn.Linear(self.embed_dim*(1-self.only_vis) + self.hidden_dim + self.in_channels * self.only_vis, self.hidden_dim)
    ...
    def forward(self, roi_features, proposals,is_decoder=False):
        ...
        obj_pre_rep = cat((roi_features,  pos_embed), -1)
        num_objs = [len(p) for p in proposals]
        if not is_decoder:
            obj_pre_rep = self.lin_obj(obj_pre_rep)
        else:
            obj_pre_rep = self.lin_edge(obj_pre_rep)

        reg_feats = self.context_obj(obj_pre_rep, num_objs)
