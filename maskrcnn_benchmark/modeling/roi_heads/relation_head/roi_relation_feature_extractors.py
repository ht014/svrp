...
class RelationFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        # separete spatial
        self.separate_spatial = False  # self.cfg.MODEL.ROI_RELATION_HEAD.CAUSAL.SEPARATE_SPATIAL
    ...
    def forward(self, x, proposals, rel_pair_idxs=None):
        ...
        for proposal, rel_pair_idx in zip(proposals, rel_pair_idxs):
            ...
            head_boxes.append(head_proposal.bbox)
            tail_boxes.append(tail_proposal.bbox)
            union_boxes.append(union_proposal.bbox)
        ...
        data = {'union_features': union_features, 'head_proposals': torch.cat(head_boxes,0), 'tail_proposals': torch.cat(tail_boxes,0),
                "union_proposals": torch.cat(union_boxes,0)}
        return data
