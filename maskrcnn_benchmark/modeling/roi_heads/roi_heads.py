...
from .relation_head.pretrain_head import build_pretrain_head
...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.cfg.MODEL.PRETRAIN_ON:
            # it may be not safe to share features due to post processing
            # During training, self.box() will return the unaltered proposals as "detections"
            # this makes the API consistent during training and testing
            x, detections, loss_contrastive = self.pretrain(features, detections, targets, logger)
            losses.update(loss_contrastive)
        if self.cfg.MODEL.RELATION_ON and not self.cfg.MODEL.PRETRAIN_ON:
            # it may be not safe to share features due to post processing
            # During training, self.box() will return the unaltered proposals as "detections"
            # this makes the API consistent during training and testing
...
def build_roi_heads(cfg, in_channels):
    ...
    if cfg.MODEL.PRETRAIN_ON:
        roi_heads.append(("pretrain", build_pretrain_head(cfg, in_channels)))
